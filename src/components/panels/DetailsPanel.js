...
const DetailsPanel = () => {
    ...
    return(
        <StyledPanel backgroundColor="#E7E7E7" flex="1">
            ...
            {currentProject === null ? <h3 style={{margin: 0, textAlign: 'center', color: 'rgba(0,0,0, .6)'}}>No project selected.</h3> : validTask.map((task) => <Task key={task.id} id={task.id} title={task.title} description={task.description} projectId={task.projectId} done={task.done} color="red"/>)}
            {toggled ?
            <div>
                <StyledInput onKeyPress={(event) => {event.key === 'Enter' && postTask(title, description, currentProject); event.key === 'Enter' && setToggled(false)}} onChange={(event) => setTitle(event.target.value)} placeholder="Task title" maxLength={24} />
                <StyledInput onKeyPress={(event) => {event.key === 'Enter' && postTask(title, description, currentProject); event.key === 'Enter' && setToggled(false)}} onChange={(event) => setDescription(event.target.value)} placeholder="Task description" maxLength={64}/>
